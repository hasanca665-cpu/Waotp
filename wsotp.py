...
class AccountManager:
    ...
    def get_user_remaining_checks(self, user_id):
        ...
        used_slots = sum(self.token_info.get(token, {}).get('usage', 0) 
    ...
    def get_next_available_token(self, user_id):
        """Get next available token for processing"""
        user_id_str = str(user_id)
    ...
    def release_token(self, token):
        ...
        if token in self.token_info:
            ...
            self.token_info[token]['usage'] = max(0, current_usage - 1)
...
async def track_status_optimized(context: CallbackContext):
    ...
    try:
        ...
        if status_code == 1 and last_status_code != 1:
            ...
            # টোকেন রিলিজ (কিন্তু ডিলিট হবে না!)
            account_manager.release_token(token)
    ...
    try:
        ...
        for user_summary in all_users_summary:
            try:
                ...
                #message += f"• 👥 Commission Rate: ${commission_rate:.3f} per count\n\n"
                ...
                #message += f"• Your Commission: ${user_summary['total_commission']:.2f}\n"
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if added:
                ...
                if actual_phone and actual_phone != phone:
                    display_phone = f"{actual_phone} (You sent: {phone})"
                else:
                    display_phone = phone
